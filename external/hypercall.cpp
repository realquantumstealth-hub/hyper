...

{
...

    {
        ...
        constexpr std::uint64_t NTOSKRNL_ALIGNMENT = 0x200000;
        ...
        constexpr std::uint64_t max_search_iterations = 0x10;
...

{
    ...
    // Get dynamic EPROCESS offsets based on Windows version
    auto offsets = os_version::get_eprocess_offsets();
    ...
    do {
        ...
        if (!read_guest_virtual(current + offsets.unique_process_id, &pid, sizeof(pid))) return 0;
        ...
        if (!read_guest_virtual(current + offsets.active_process_links, &next_link, sizeof(next_link))) return 0;



        current = next_link - offsets.active_process_links;
...
std::uint64_t get_process_base_by_pid_from_initial(std::uint64_t target_pid, std::uint64_t ps_initial_system_process, cr3 kernel_cr3) {
    ...
    do {
        ...
        if (pid == target_pid) {
            ...
            if (read_guest_virtual(current + offsets.section_base_address, &section_base_address, sizeof(section_base_address))) {
...
case hypercall_type_t::query_hypervisor_pfn_info:
    ...
    case hypercall_type_t::get_tbi_dll_info:
    {
        std::uint64_t output_buffer_gva = trap_frame->rdx;
        ...
        // Get TBI info from discovery module
        tbi_info_t tbi_info = tbi_discovery::get_tbi_info();
        ...
        std::uint64_t bytes_written = memory_manager::operate_on_guest_virtual_memory(
            slat_cr3, &tbi_info, output_buffer_gva, guest_cr3,
            sizeof(tbi_info_t), memory_operation_t::write_operation);

        trap_frame->rax = (bytes_written == sizeof(tbi_info_t)) ? STATUS_SUCCESS : STATUS_UNSUCCESSFUL;
        break;
    }

    case hypercall_type_t::get_tbi_dll_base:
    {
        // Return the TBI.dll base address directly
        trap_frame->rax = tbi_discovery::get_tbi_base();
        break;
    }
