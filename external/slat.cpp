...
{
#ifdef _INTELMACHINE
	// Check if PML4E is present - if not, EPT doesn't have this region mapped yet
	if (pml4e->read_access == 0 && pml4e->write_access == 0 && pml4e->execute_access == 0)
	{
		return nullptr; // Entry not present - page not mapped in EPT
	}
#else
	if (pml4e->present == 0)
	{
...
{
#ifdef _INTELMACHINE
	// Check if PDPTE is present
	if (pdpte->read_access == 0 && pdpte->write_access == 0 && pdpte->execute_access == 0)
	{
		return nullptr; // Entry not present
	}
#else
	if (pdpte->present == 0)
	{
...
slat_pte* slat_get_pte(slat_pde* pde, virtual_address_t guest_physical_address)
{
	...
	if (pde->read_access == 0 && pde->write_access == 0 && pde->execute_access == 0)
	{
		return nullptr; // Entry not present
	}
#else
...
#ifdef _INTELMACHINE
// Ensure EPT has entry for guest physical address (create if missing)
std::uint8_t ensure_ept_entry(cr3 ept_cr3, virtual_address_t guest_physical_address)
{
	slat_pml4e* pml4 = reinterpret_cast<slat_pml4e*>(memory_manager::map_host_physical(ept_cr3.address_of_page_directory << 12));
	slat_pml4e* pml4e = &pml4[guest_physical_address.pml4_idx];

	// Check if PML4E needs to be created
	if (pml4e->read_access == 0 && pml4e->write_access == 0 && pml4e->execute_access == 0)
	{
		// Allocate PDPT
		slat_pdpte* pdpt = static_cast<slat_pdpte*>(heap_manager::allocate_page());
		if (pdpt == nullptr) return 0;

		crt::set_memory(pdpt, 0, 0x1000);
		std::uint64_t pdpt_phys = memory_manager::unmap_host_physical(reinterpret_cast<std::uint64_t>(pdpt));

		pml4e->page_frame_number = pdpt_phys >> 12;
		pml4e->read_access = 1;
	...
	// Manually walk to PDPTE (don't use slat_get_pdpte which checks permissions)
	slat_pdpte* pdpt = reinterpret_cast<slat_pdpte*>(memory_manager::map_host_physical(pml4e->page_frame_number << 12));
	slat_pdpte* pdpte = &pdpt[guest_physical_address.pdpt_idx];

	// Check if PDPTE needs to be created
	if (pdpte->read_access == 0 && pdpte->write_access == 0 && pdpte->execute_access == 0)
	{
		// Allocate PD
		slat_pde* pd = static_cast<slat_pde*>(heap_manager::allocate_page());
		if (pd == nullptr) return 0;

		crt::set_memory(pd, 0, 0x1000);
		std::uint64_t pd_phys = memory_manager::unmap_host_physical(reinterpret_cast<std::uint64_t>(pd));

		pdpte->page_frame_number = pd_phys >> 12;
		pdpte->read_access = 1;
	...
	// Manually walk to PDE
	slat_pde* pd = reinterpret_cast<slat_pde*>(memory_manager::map_host_physical(pdpte->page_frame_number << 12));
	slat_pde* pde = &pd[guest_physical_address.pd_idx];

	// Check if PDE needs to be created
	if (pde->read_access == 0 && pde->write_access == 0 && pde->execute_access == 0)
	{
		// Allocate PT
		...
		crt::set_memory(pt, 0, 0x1000);
		std::uint64_t pt_phys = memory_manager::unmap_host_physical(reinterpret_cast<std::uint64_t>(pt));

		pde->page_frame_number = pt_phys >> 12;
		pde->read_access = 1;
		pde->write_access = 1;
	...
	// Manually walk to PTE
	slat_pte* pt = reinterpret_cast<slat_pte*>(memory_manager::map_host_physical(pde->page_frame_number << 12));
	slat_pte* pte = &pt[guest_physical_address.pt_idx];

	// Check if PTE needs to be created/mapped to actual page
	if (pte->read_access == 0 && pte->write_access == 0 && pte->execute_access == 0)
	{
		// Map to the actual guest physical page
		pte->page_frame_number = guest_physical_address.address >> 12;
		pte->read_access = 1;
		pte->write_access = 1;
		pte->execute_access = 1;
		pte->user_mode_execute = 1;
		pte->memory_type = 6; // Write-back
	}
...
// Lazily populate hook NPT by copying structure from hyperv NPT
std::uint8_t ensure_hook_npt_entry(virtual_address_t target_guest_physical_address)
{
	// Get PML4 entry from hyperv NPT
	slat_pml4e* hyperv_pml4e = slat_get_pml4e(hyperv_nested_cr3, target_guest_physical_address);
	slat_pml4e* hook_pml4e = slat_get_pml4e(hook_nested_cr3, target_guest_physical_address);

	if (!hyperv_pml4e || !hook_pml4e) return 0;

	// Copy PML4 entry if not present
	if (hook_pml4e->present == 0 && hyperv_pml4e->present != 0)
	{
		pdpte_64* hook_pdpt = static_cast<pdpte_64*>(heap_manager::allocate_page());
		if (!hook_pdpt) return 0;

		crt::set_memory(hook_pdpt, 0, 0x1000);
		std::uint64_t hook_pdpt_physical = memory_manager::unmap_host_physical(reinterpret_cast<std::uint64_t>(hook_pdpt));

		hook_pml4e->present = 1;
		hook_pml4e->write = 1;
		hook_pml4e->page_frame_number = hook_pdpt_physical >> 12;
	}

	// Get PDPT entry
	slat_pdpte* hyperv_pdpte = slat_get_pdpte(hyperv_pml4e, target_guest_physical_address);
	slat_pdpte* hook_pdpte = slat_get_pdpte(hook_pml4e, target_guest_physical_address);

	if (!hyperv_pdpte || !hook_pdpte) return 0;

	// Copy PDPT entry if not present
	if (hook_pdpte->present == 0 && hyperv_pdpte->present != 0)
	{
		slat_pdpte_1gb* large_pdpte = reinterpret_cast<slat_pdpte_1gb*>(hyperv_pdpte);
		if (large_pdpte->large_page == 1)
		{
			// Copy 1GB page directly
			*hook_pdpte = *hyperv_pdpte;
			return 1;
		}

		pde_64* hook_pd = static_cast<pde_64*>(heap_manager::allocate_page());
		if (!hook_pd) return 0;

		crt::set_memory(hook_pd, 0, 0x1000);
		std::uint64_t hook_pd_physical = memory_manager::unmap_host_physical(reinterpret_cast<std::uint64_t>(hook_pd));

		hook_pdpte->present = 1;
		hook_pdpte->write = 1;
		hook_pdpte->page_frame_number = hook_pd_physical >> 12;
	}

	// Get PDE entry
	slat_pde* hyperv_pde = slat_get_pde(hyperv_pdpte, target_guest_physical_address);
	slat_pde* hook_pde = slat_get_pde(hook_pdpte, target_guest_physical_address);

	if (!hyperv_pde || !hook_pde) return 0;

	// Copy PDE entry if not present
	if (hook_pde->present == 0 && hyperv_pde->present != 0)
	{
		slat_pde_2mb* large_pde = reinterpret_cast<slat_pde_2mb*>(hyperv_pde);
		if (large_pde->large_page == 1)
		{
			// Copy 2MB page directly
			*hook_pde = *hyperv_pde;
			return 1;
		}

		pte_64* hook_pt = static_cast<pte_64*>(heap_manager::allocate_page());
		if (!hook_pt) return 0;

		crt::set_memory(hook_pt, 0, 0x1000);
		std::uint64_t hook_pt_physical = memory_manager::unmap_host_physical(reinterpret_cast<std::uint64_t>(hook_pt));

		hook_pde->present = 1;
		hook_pde->write = 1;
		hook_pde->page_frame_number = hook_pt_physical >> 12;

		// Copy all PT entries from hyperv NPT
		slat_pte* hyperv_pt = slat_get_pte(hyperv_pde, target_guest_physical_address);
		if (hyperv_pt)
		{
			pte_64* hyperv_pt_base = reinterpret_cast<pte_64*>(memory_manager::map_host_physical(hyperv_pde->page_frame_number << 12));
			for (int i = 0; i < 512; i++)
			{
				hook_pt[i] = hyperv_pt_base[i];
			}
		}
	}
...
{
#ifdef _INTELMACHINE
	// SIMPLIFIED: Use single EPT instead of dual-EPT (like AMD)
	// Just use the current EPT from Hyper-V directly
	cr3 current_ept = get_cr3();

	// Set both to the same EPT - we'll hook in-place
	clean_ept_cr3 = current_ept;
	hooked_ept_cr3 = current_ept;

	current_ept_is_hooked = 0;
#else
	vmcb_t* const vmcb = arch::get_vmcb();

	// Save the Hyper-V nested CR3 - this is our "clean" NPT
	hyperv_nested_cr3 = vmcb->control.nested_cr3;

	// Don't build hook NPT yet - it will be lazily built when first hook is added
	// Just initialize ASID for the clean NPT
	vmcb->control.asid = clean_asid;
...
std::uint64_t slat::add_slat_code_hook(cr3 slat_cr3, virtual_address_t target_guest_physical_address, virtual_address_t shadow_page_guest_physical_address)
{
...
#ifdef _INTELMACHINE
	// SIMPLIFIED: Single EPT approach (like AMD)
	std::uint8_t paging_split_state = 0;

	// Use the current EPT directly
	cr3 current_ept = get_cr3();

	// DON'T use ensure_ept_entry() - it creates wrong identity mappings for user-mode pages
	// Instead, check if Hyper-V has already mapped this page in its EPT
	// Get/create PTE (force split if large page)
	slat_pte* target_pte = slat_get_pte(current_ept, target_guest_physical_address, 1, &paging_split_state);

	if (target_pte == nullptr)
	{
		// Page not in Hyper-V's EPT yet - this is expected for user-mode pages
		// The page needs to be accessed by the guest first to trigger Hyper-V to map it
		hook_mutex.release();
		return 0;
	}

	// Check if PTE is actually mapped (has permissions set)
	if (target_pte->read_access == 0 && target_pte->write_access == 0 && target_pte->execute_access == 0)
	{
		// PTE exists but is not mapped - page not accessible
		hook_mutex.release();
		return 0;
	}
#else
	...
	// Ensure hook NPT has the necessary structure copied from hyperv NPT
	if (ensure_hook_npt_entry(target_guest_physical_address) == 0)
	{
		hook_mutex.release();
		return 0;
	}
...
#ifdef _INTELMACHINE
	// SIMPLIFIED: Save original state and modify in place
	...
	// Modify PTE: execute-only, point to shadow page
...
{
#ifdef _INTELMACHINE
	// SIMPLIFIED: Restore original PTE values
	cr3 current_ept = slat::get_cr3();
	slat_pte* target_pte = slat_get_pte(current_ept, target_guest_physical_address);
	...
	// Restore original PFN and permissions
	target_pte->page_frame_number = hook_entry->get_original_pfn();
	target_pte->read_access = hook_entry->get_original_read_access();
	target_pte->write_access = hook_entry->get_original_write_access();
	target_pte->execute_access = hook_entry->get_original_execute_access();

	if (does_hook_need_merge(hook_entry, target_guest_physical_address) == 1)
	{
		slat_merge_4kb_pt(current_ept, target_guest_physical_address);
	}
#else
