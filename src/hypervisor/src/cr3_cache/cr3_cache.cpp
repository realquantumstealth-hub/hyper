...
namespace cr3_cache
{
    ...
    void try_cache_on_vmexit()
    {
        ...
        // Log every 100000 samples to show it's working
        if (g_stats.total_samples % 100000 == 0)
        {
            trap_frame_log_t log = {};
            log.r15 = 0xC301; // Sample count marker
            log.r14 = g_stats.total_samples;
            log.r13 = g_stats.ring3_samples;
            logs::add_log(log);
        }
        ...
        // Log first ring3 sample
        if (g_stats.ring3_samples == 1)
        {
            trap_frame_log_t log = {};
            log.r15 = 0xC302; // First ring3 sample
            log.r14 = cs_selector;
            log.r13 = g_target_pid;
            logs::add_log(log);
        }
        ...
        // Log PID read attempts every 1000 ring3 samples
        if (g_stats.ring3_samples % 1000 == 0)
        {
            trap_frame_log_t log = {};
            log.r15 = 0xC303; // PID check marker
            log.r14 = current_pid;
            log.r13 = g_target_pid;
            logs::add_log(log);
        }
        ...
        // Log first target PID hit
        if (g_stats.target_pid_hits == 1)
        {
            trap_frame_log_t log = {};
            log.r15 = 0xC304; // First target PID hit
            log.r14 = current_pid;
            log.r13 = g_target_pid;
            logs::add_log(log);
        }
